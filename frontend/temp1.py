...
def query_by_txnid(user, txnid):
    ...
    try:
        #subprocess.call("cd "+FABRIC_DIR,shell=True)
        output = subprocess.check_output(
            [NODE_PATH, FABRIC_DIR + "/query.js", "QueryTxn", user, txnid], cwd=FABRIC_DIR).decode()
        # Looking for index where dictionary starts
        keyword_before_dict = "result is: "
        start_of_dict_index = output.find(
            keyword_before_dict) + len(keyword_before_dict)
...
def query_by_email(user, email):
    ...
    try:
        #subprocess.call("cd "+FABRIC_DIR,shell=True)
        output = subprocess.check_output(
            [NODE_PATH, FABRIC_DIR + "/query.js", "QueryAllTxn", user, email], cwd=FABRIC_DIR).decode()
...
def query_all_txn(user):
    ...
    try:
        #subprocess.call("cd "+FABRIC_DIR,shell=True)
        output = subprocess.check_output(
            [NODE_PATH, FABRIC_DIR + "/query.js", "QueryAllTxn", user], cwd=FABRIC_DIR).decode()
        # Looking for index where list starts
        keyword_before_dict = "result is: "
        start_of_dict_index = output.find(
            keyword_before_dict) + len(keyword_before_dict)
...
def approve_txn(user, txnid):
    ...
    try:
        #subprocess.call("cd "+FABRIC_DIR,shell=True)
        output = subprocess.check_output(
            [NODE_PATH, FABRIC_DIR + "/invoke.js", "CreateApprovalTxn", txnid, user], cwd=FABRIC_DIR).decode()
...
#     if res == 0:
#         flash(
#             'Registration successful! Check your email inbox/spam for password', 'success')
...
def getbalance_post():
    ...
    if bal == -1:
        ...
        flash('The current balance of ' +
              request.form['email'] + ' is '+bal+".", 'notification')
        return redirect('/getbalance')
